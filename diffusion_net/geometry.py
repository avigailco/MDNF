...
def build_grad(verts, edges, edge_tangent_vectors):
    ...
    Given real inputs at vertices, produces a complex (vector value) at vertices giving the gradient.
...
def get_all_pairs_geodesic_distance(verts_np, faces_np, geodesic_cache_dir=None):
    ...
    if geodesic_cache_dir is not None:
        ...
        while True:
            ...
            try:
                # npzfile = np.load(search_path, allow_pickle=True)
                npzfile = np.load(search_path, mmap_mode='r')
    ...
    if not found:
        ...
        # NUM_WORKERS = 1  # Match SLURM --cpus-per-task=8
        # pool = None  # <-- define it ahead of time
        # print("CPUs:", os.cpu_count())
        # try:
        #     from multiprocessing import get_context
        #     ctx = get_context("spawn")
        #     pool = ctx.Pool(processes=NUM_WORKERS)  # Explicit control
        #     engine = AllPairsGeodesicEngine(verts_np, faces_np)
        #     try:
        #         outputs = pool.map(engine, range(N))
        #     except Exception as e:
        #         print("❌ Error occurred during multiprocessing:")
        #         import traceback
        #         traceback.print_exc()
        #         raise e
        # finally:
