...
class LearnedTimeDiffusion(nn.Module):
    ...
    def __init__(self, C_inout, method='spectral', init_t=0.0):
        ...
        nn.init.constant_(self.diffusion_time, init_t)
...
class DiffusionNetBlock(nn.Module):
    ...
    def __init__(self, C_width, mlp_hidden_dims,
        ...
        # Diffusion block
        self.diffusion = LearnedTimeDiffusion(self.C_width, method=diffusion_method, init_t=init_t)
...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, C_in, C_out, C_width=128, N_block=4, last_activation=None, outputs_at='vertices',
                 mlp_hidden_dims=None, dropout=True, with_gradient_features=True, with_gradient_rotations=True,
                 diffusion_method='spectral', init_t=0.0):
        """
        ...
        Parameters:
            ...
            init_t (float):                 initial value for diffusion time parameter
        """
    ...
    def forward(self, x_in, mass, L=None, evals=None, evecs=None, gradX=None, gradY=None, edges=None, faces=None):
        ...
        #     plt.plot(toNP(x_out[0,:,i]), color=color, label=f"{color}")
        # plt.legend()
        # plt.show()
        ...
        # ps.init()
        # ps.set_ground_plane_mode('shadow_only')  # Use 'none' if you do not want any ground plane
        # ps_mesh = ps.register_surface_mesh("Mesh", toNP(verts), toNP(faces))
        # for i, color in zip([0,1,2], ["r","g","b"]):
        #     rgb_values = colormap(x_out[0,:,i])[..., :3].squeeze(1)  # The last column is alpha, so we exclude it
        #     ps_mesh.add_color_quantity(f"{color}", rgb_values, enabled=True)
        # ps.show()
